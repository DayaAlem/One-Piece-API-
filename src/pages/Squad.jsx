...
export const Squad = () => {
  ...
  const handleSearchSubmit = (event) => {
    ...
    const filteredPersonnages = displayedPersonnages.filter((personnage) => {
      return personnage.french_name.toLowerCase().includes(searchTerm.toLowerCase());
    });
  ...
  return (
    <div>
      ...
      {displayedPersonnages.map(({id, french_name, total_prime, number, status, affiliation})=>( 
        <div className='item' key = {id}>
          <Link to={`/crews/${id}`}>                   
          <p className='titlesquad'>{french_name}</p>
          <p className='totalprime'>Prime Total de L'équipage : {total_prime}</p>
